...
const MOBILE_NET_D = vline(160, 182, 226);
const MOBILE_SEGS: Seg[] = [
  { id: "m-fb", d: MOBILE_NET_D, color: FB, dur: 1.4, cycle: 4.5, delay: 0, arriveNode: "whatsapp", hiddenTrack: true },
  { id: "m-ig", d: MOBILE_NET_D, color: IG, dur: 1.4, cycle: 4.5, delay: 1.5, arriveNode: "whatsapp", hiddenTrack: true },
  { id: "m-tt", d: MOBILE_NET_D, color: TT, dur: 1.4, cycle: 4.5, delay: 3.0, arriveNode: "whatsapp", hiddenTrack: true },
  { id: "m-wa", d: vline(160, 294, 340), color: GREEN, dur: 1.5, cycle: 3.2, delay: 0.7, arriveNode: "clinvex", hiddenTrack: true },
  { id: "m-mod", d: vline(160, 444, 476), color: GOLD, dur: 1.2, cycle: 3.0, delay: 0.3, hiddenTrack: true },
  { id: "m-cli", d: vline(160, 704, 744), color: GOLD, dur: 1.4, cycle: 3.4, delay: 1.1, arriveNode: "clientes", hiddenTrack: true },
];

const BASE_TOTAL = 47350;
...
function useFlowEngine(segs: Seg[], active: boolean, onArrive: (node: string) => void) {
...
function useMoney(active: boolean) {
...
function FillIcon({ d, x, y, size = 16, color }: { d: string; x: number; y: number; size?: number; color: string }) {
  return (
...
function GlowRect({ x, y, w, h, on, color = "var(--accent-400)" }: { x: number; y: number; w: number; h: number; on: boolean; color?: string }) {
  return (
    <motion.rect
      x={x}
      y={y}
...
function GlowCircle({ cx, cy, r, on, color }: { cx: number; cy: number; r: number; on: boolean; color: string }) {
  return (
    <motion.circle
      cx={cx}
      cy={cy}
...
function SegmentDots({ seg, setDotRef, withTrail }: { seg: Seg; setDotRef: (id: string, i: number) => (el: SVGCircleElement | null) => void; withTrail: boolean }) {
  return (
    <g>
...
function MoneyFloaters({ floaters, cx, y }: { floaters: { id: number; dx: number }[]; cx: number; y: number }) {
  return (
    <>
      {floaters.map((f) => (
        <motion.text
...
function Counter({ total, cx, y, valueSize = 17 }: { total: number; cx: number; y: number; valueSize?: number }) {
  return (
    <g>
      <text
        x={cx}
      ...
      <text x={cx} y={y + 20} textAnchor="middle" fontSize="10.5" fill="var(--text-muted)" fontFamily="var(--font-body)">
        em pacotes fechados
      </text>
    </g>
  );
...
/* ── Mobile ────────────────────────────────────────────────────────── */
const M_NETWORKS = [
  { id: "facebook", label: "Facebook", y: 42 },
  { id: "instagram", label: "Instagram", y: 102 },
  { id: "tiktok", label: "TikTok", y: 162 },
];

const M_MODULES = [
  { id: "listas", label: "Listas", y: 500 },
  { id: "ia", label: "Inteligência Artificial", y: 560 },
  { id: "disparos", label: "Disparos", y: 620 },
  { id: "automacoes", label: "Automações", y: 680 },
];

function MobileFlow({ prefersReduced }: { prefersReduced: boolean }) {
  const svgRef = useRef<SVGSVGElement>(null);
  const inView = useInView(svgRef, { amount: 0.15 });
  const active = inView && !prefersReduced;
  const { glows, glow } = useGlows();
  const { total, floaters, onSale } = useMoney(active);
  ...
  const { setPathRef, setDotRef } = useFlowEngine(MOBILE_SEGS, active, onArrive);
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id="clx-glow-m" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor={GOLD} stopOpacity="0.28" />
          <stop offset="70%" stopColor={GOLD} stopOpacity="0.06" />
          <stop offset="100%" stopColor={GOLD} stopOpacity="0" />
        </radialGradient>
      ...
      {/* Trilha vertical única */}
      <line x1={160} y1={182} x2={160} y2={748} stroke="var(--border-default)" strokeWidth="1.25" />

      {/* Segmentos invisíveis para as partículas */}
      {MOBILE_SEGS.map((s) => (
        <path key={s.id} ref={setPathRef(s.id)} d={s.d} fill="none" stroke="none" />
      ))}
      {!prefersReduced &&
        MOBILE_SEGS.map((s) => <SegmentDots key={`dots-${s.id}`} seg={s} setDotRef={setDotRef} withTrail={false} />)}

      {/* Redes */}
      {M_NETWORKS.map((n) => (
        <g key={n.id}>
          <rect x={60} y={n.y - 20} width={200} height={40} rx={10} fill="var(--bg-elevated)" stroke="var(--border-default)" strokeWidth="1" />
          <FillIcon d={GLYPHS[n.id]} x={76} y={n.y - 8} size={16} color="var(--text-secondary)" />
          <text x={102} y={n.y + 4} fontSize="12" fill="var(--text-secondary)" fontFamily="var(--font-body)">
            {n.label}
          </text>
        </g>
      ))}
      ...
      {/* WhatsApp */}
      <g>
        <circle cx={160} cy={260} r={30} fill="rgba(37, 211, 102, 0.10)" stroke={GREEN} strokeWidth="1.5" />
        <FillIcon d={GLYPHS.whatsapp} x={147} y={247} size={26} color={GREEN} />
        <GlowCircle cx={160} cy={260} r={34} on={!!glows.whatsapp} color={GREEN} />
      </g>
      ...
      <g>
        <motion.circle
          ...
          transition={active ? { duration: 4, repeat: Infinity, ease: "easeInOut" } : { duration: 0.3 }}
        />
        <circle cx={160} cy={392} r={48} fill="var(--bg-elevated)" stroke="var(--accent-400)" strokeWidth="2" />
        <GlowCircle cx={160} cy={392} r={53} on={!!glows.clinvex} color={GOLD} />
        <text
          x={160}
          y={399}
          textAnchor="middle"
      ...
      {M_MODULES.map((m) => (
        <g key={m.id}>
          <rect x={44} y={m.y - 20} width={232} height={40} rx={10} fill="var(--bg-elevated)" stroke="var(--border-default)" strokeWidth="1" />
          <StrokeIcon x={62} y={m.y - 8} size={16}>
            {MODULE_ICONS[m.id]}
          </StrokeIcon>
          <text x={90} y={m.y + 4} fontSize="12.5" fill="var(--text-secondary)" fontFamily="var(--font-body)">
            {m.label}
          </text>
          <GlowRect x={44} y={m.y - 20} w={232} h={40} on={!!glows[m.id]} />
        </g>
      ))}

      {/* Clientes */}
      <g>
        <circle cx={160} cy={780} r={32} fill="var(--bg-elevated)" stroke={SUCCESS} strokeWidth="1.5" />
        <StrokeIcon x={147} y={767} size={26} color={SUCCESS}>
          {USERS_ICON}
        </StrokeIcon>
        <GlowCircle cx={160} cy={780} r={36} on={!!glows.clientes} color={SUCCESS} />
        {!prefersReduced && <MoneyFloaters floaters={floaters} cx={218} y={786} />}
        <Counter total={total} cx={160} y={840} valueSize={16} />
      </g>
    </svg>
  );
}
...
export default function EcossistemaSVG() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <DesktopFlow prefersReduced={prefersReduced} />
      <MobileFlow prefersReduced={prefersReduced} />
    </div>
  );
}
